...
def generate_cards_html():
    ...
    if not HISTORY_READY and not sorted_news:
        return "<div class='empty-state'>⏳ Завантаження новин, зачекайте кілька секунд... Сторінка оновиться автоматично.</div>"
    if not sorted_news:
        return "<div class='empty-state'>📭 Немає свіжих новин за останні години.</div>"
        ...
        <script>
            async function updateNews() {{
                try {{
                    ...
                    console.error('Помилка оновлення новин:', err);
                }}
            }}
